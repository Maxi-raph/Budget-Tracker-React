...
const Transactions = () => {
   const {
    ...
    selectCategory,handleDesc,addTransaction,selectRef} = useTransaction()
   ...
   return ( <div className=" max-w-5xl mx-auto p-5 rounded-lg bg-white shadow-lg scroll-mt-22" id="transactions">
        ...
        <div className="flex flex-col-reverse gap-6 lg:flex-row lg:items-start sm:flex-col-reverse sm:items-center">
            <TransactionList />
            <div className="bg-gray-200 rounded-lg shadow-lg lg:w-[40%] p-3 sm:w-full">
                <div className="sm:flex justify-around items-center">
                    ...
                    <div className="flex justify-end mb-4 sm:justify-start sm:mb-0">
                        <span data-name='Expense' className={`block p-2 text-sm border border-gray-400  rounded-l-sm rounded-tl-sm cursor-pointer font-medium ${!chooseType?'bg-gray-50':''}`} onClick={(e)=>handleTypeClick(e)}>Expense</span>
                        <span data-name='Income' className={`block p-2 text-sm border border-gray-400 rounded-r-sm rounded-br-sm cursor-pointer font-medium ${chooseType?'bg-gray-50':''}`} onClick={(e)=>handleTypeClick(e)}>Income</span>
                    </div>
                </div>
                <div className="grid sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input className="w-full p-3 font-bold bg-white rounded-lg placeholder:text-gray-400" placeholder="0.00" type="number"  value={transaction['amount']} onChange={(e)=>handleAmt(e)}/>
                    </div>
                    <div>
                        <span className="text-gray-400 mb-3 block font-semibold text">Category  <span className="inline-block text-red-400 font-semibold">*</span></span>
                           {!chooseType
                           ?
                        <select ref={selectRef} className="block p-3 font-bold bg-white cursor-pointer rounded-lg w-full" value={transaction['category']} onChange={(e)=>selectCategory(e)}>
                            <optgroup>
                    ...
                    <div>
                        ...
                        <textarea className="w-full p-3 font-bold outline-gray-100 bg-white rounded-lg placeholder:text-gray-400" placeholder="Groceries..." maxLength={13} value={transaction['description']}  onChange={(e)=>handleDesc(e)}/>
                    </div>  
                </div>
                <button className="w-full p-2 bg-gray-900 text-white cursor-pointer  rounded-lg shadow-lg mt-4 flex justify-center items-center gap-2" onClick={addTransaction}><FaPlus />Add {chooseType?'Income':'Expense'}</button>
            </div>
        </div>
    </div> );
}
